...
(function(){
  ...
  window.showCard = function(props, latlng, layerTitle, layerKey){
    ...
    { const ref = props.asset_id ? ("asset:"+props.asset_id) : ((layerKey||layerTitle||"item")+":"+(props.key||props.name||props.tree_name||"?")); const holder=document.createElement("div"); document.getElementById("cbody").appendChild(holder); let tries=0; const go=()=>{ if(window.edrItem) edrItem(ref, holder, {latlng}); else if(tries++<25) setTimeout(go,200); }; go(); }
