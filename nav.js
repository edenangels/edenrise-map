// App shell: navigation + Share sheet (copy · WhatsApp · QR · print) + toast. Loads ui.css tokens.
(function(){
  if(!document.querySelector('link[href="ui.css"]')){ const l=document.createElement("link"); l.rel="stylesheet"; l.href="ui.css"; document.head.appendChild(l); }
  const here = location.pathname.split("/").pop() || "index.html";
  let LANG = "pt"; try{ LANG = localStorage.getItem("edr_lang") || ((navigator.language||"pt").toLowerCase().startsWith("pt")?"pt":"en"); }catch(e){}
  const T = {pt:{share:"PARTILHAR",title:"Partilhar esta vista",sub:"O link abre exatamente o que está no ecrã — posição, camadas e idioma.",copy:"Copiar link",copied:"Link copiado",wa:"WhatsApp",wasub:"enviar à equipa",qr:"Mostrar QR",qrsub:"para telemóvel",print:"Imprimir / PDF",printsub:"esta página",asset:"Link do ativo",assetsub:"ficha aberta",view:"vista atual"},
             en:{share:"SHARE",title:"Share this view",sub:"The link opens exactly what's on screen — position, layers and language.",copy:"Copy link",copied:"Link copied",wa:"WhatsApp",wasub:"send to the team",qr:"Show QR",qrsub:"for a phone",print:"Print / PDF",printsub:"this page",asset:"Asset link",assetsub:"open card",view:"current view"}};
  const t = T[LANG];
  const items = [["index.html",{pt:"Mapa",en:"Map"}],["3d.html",{pt:"3D",en:"3D"}],["dashboard.html",{pt:"Painel",en:"Dashboard"}],["climate.html",{pt:"Clima",en:"Climate"}],["report.html",{pt:"Relatório",en:"Report"}],["guest.html",{pt:"Hóspedes",en:"Guests"}],["labels.html",{pt:"Etiquetas",en:"Labels"}],["help.html",{pt:"Ajuda",en:"Help"}]];
  const SHOW = {"climate.html":"climate","labels.html":"labels"};   // pages that exist only where the products exist
  const visible = items.filter(([h])=>!SHOW[h] || (window.SITE && SITE.flags && SITE.flags[SHOW[h]]));
  const sh = (h) => window.siteHref ? siteHref(h) : h;
  function siteMenu(){ const S = window.SITES || {}, ids = Object.keys(S); if(ids.length < 2 || !window.SITE) return "";
    return `<div class="site"><button class="sitebtn" id="sitebtn" aria-haspopup="true" aria-expanded="false" title="${LANG==="en"?"Switch property":"Mudar de propriedade"}">${SITE.name} ▾</button><div class="sitemenu" id="sitemenu" hidden>${ids.map(k=>{ const s=S[k]; const page=(SHOW[here] && !(s.flags&&s.flags[SHOW[here]])) ? "index.html" : here; const href=page+(k==="edenrise"?"":"?site="+k); return `<a href="${href}" class="${k===SITE_ID?"on":""}">${s.name}<small>${s.place[LANG]}</small></a>`; }).join("")}</div></div>`; }
  const nav = document.createElement("nav"); nav.id = "appnav";
  nav.innerHTML = `<a class="brand" href="${sh("index.html")}"><span class="mark"></span><span class="t">EdenRise</span></a>${siteMenu()}<div class="links">` +
    visible.map(([h,l])=>`<a href="${sh(h)}" class="${h===here?"on":""}">${l[LANG]}</a>`).join("") + `</div>` +
    `<span class="sp"></span><button class="edit" id="nav-edit" aria-label="${LANG==="en"?"Edit the map":"Editar o mapa"}" title="${LANG==="en"?"Add or change items on the map":"Adicionar ou alterar itens no mapa"}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><path d="M12 20h9M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4z"/></svg><span>${LANG==="en"?"EDIT":"EDITAR"}</span></button><button class="share" id="navshare" aria-label="${t.share}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 12v7a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1v-7"/><path d="M16 6l-4-4-4 4"/><path d="M12 2v13"/></svg><span>${t.share}</span></button>`;
  document.body.prepend(nav);
  (()=>{ const sb=nav.querySelector("#sitebtn"); if(!sb) return; const sm=nav.querySelector("#sitemenu"); sb.onclick=e=>{ e.stopPropagation(); sm.hidden=!sm.hidden; sb.setAttribute("aria-expanded", String(!sm.hidden)); }; document.addEventListener("click",()=>{ sm.hidden=true; sb.setAttribute("aria-expanded","false"); }); })();
  try{ const on=nav.querySelector(".links a.on"); if(on&&window.innerWidth<=700) on.scrollIntoView({inline:"center",block:"nearest"}); }catch(e){}
  window.toast = function(msg){ const el=document.createElement("div"); el.className="toast"; el.textContent=msg; document.body.appendChild(el); setTimeout(()=>el.remove(), 1800); };
  function link(){ return window.viewLink ? window.viewLink() : location.href; }
  function openSheet(){
    const url = link(); const assetHash = location.hash.match(/#(asset|work|at)=[^&]+/);
    const bg = document.createElement("div"); bg.className = "sheet-bg";
    bg.innerHTML = `<div class="sheet" role="dialog" aria-label="${t.title}"><button class="close" aria-label="close">✕</button>
      <h3>${t.title}</h3><div class="sub">${t.sub}</div>
      <div class="what"><div class="qr" id="shqr"></div><div><div class="eyebrow">${document.title.replace("EdenRise — ","")} · ${t.view}</div><div class="lnk">${url.replace(/^https?:\/\//,"")}</div></div></div>
      <div class="opts">
        <button class="opt primary" id="shcopy"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="12" height="12" rx="2"/><path d="M5 15V5a2 2 0 0 1 2-2h10"/></svg><span>${t.copy}<small>${url.length>60?url.slice(0,58)+"…":url}</small></span></button>
        <a class="opt" target="_blank" href="https://wa.me/?text=${encodeURIComponent(url)}"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2a10 10 0 0 0-8.6 15.1L2 22l5-1.3A10 10 0 1 0 12 2zm0 18a8 8 0 0 1-4.1-1.1l-.3-.2-3 .8.8-2.9-.2-.3A8 8 0 1 1 12 20zm4.4-6c-.2-.1-1.4-.7-1.6-.8s-.4-.1-.5.1-.6.8-.8 1-.3.2-.5.1a6.6 6.6 0 0 1-3.3-2.9c-.2-.4.2-.4.7-1.3.1-.2 0-.3 0-.4l-.7-1.8c-.2-.5-.4-.4-.5-.4h-.5a1 1 0 0 0-.7.3 3 3 0 0 0-.9 2.2 5.2 5.2 0 0 0 1.1 2.8 12 12 0 0 0 4.6 4c1.7.7 2 .6 2.6.5a2.3 2.3 0 0 0 1.5-1 1.8 1.8 0 0 0 .1-1c0-.2-.2-.2-.5-.4z"/></svg><span>${t.wa}<small>${t.wasub}</small></span></a>
        ${assetHash?`<button class="opt" id="shasset"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 22s7-7.6 7-13a7 7 0 1 0-14 0c0 5.4 7 13 7 13z"/><circle cx="12" cy="9" r="2.5"/></svg><span>${t.asset}<small>${assetHash[0]}</small></span></button>`:""}
        <button class="opt" id="shprint"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 9V3h12v6M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg><span>${t.print}<small>${t.printsub}</small></span></button>
      </div></div>`;
    document.body.appendChild(bg);
    const close = ()=>bg.remove(); bg.querySelector(".close").onclick = close; bg.addEventListener("click", e=>{ if(e.target===bg) close(); });
    document.addEventListener("keydown", function esc(e){ if(e.key==="Escape"){ close(); document.removeEventListener("keydown", esc); } });
    const copy = (u)=>{ try{ navigator.clipboard.writeText(u); }catch(e){} toast(t.copied); close(); };
    bg.querySelector("#shcopy").onclick = ()=>copy(url);
    const a = bg.querySelector("#shasset"); if(a) a.onclick = ()=>copy(location.origin + location.pathname + assetHash[0]);
    bg.querySelector("#shprint").onclick = ()=>{ close(); setTimeout(()=>window.print(), 100); };
    const drawQR = ()=>{ try{ new QRCode(bg.querySelector("#shqr"), {text:url, width:56, height:56, correctLevel:QRCode.CorrectLevel.L}); }catch(e){} };
    if(typeof QRCode==="undefined"){ const s=document.createElement("script"); s.src="https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js"; s.onload=drawQR; document.head.appendChild(s); } else drawQR();
  }
  document.getElementById("navshare").onclick = openSheet;
  document.getElementById("nav-edit").onclick = ()=>{ if(window.edrEdit) window.edrEdit.toggle(); else location.href = sh("index.html")+"#edit"; };
  const est = document.createElement("style"); est.textContent = `.nav .site{position:relative;margin-left:10px}.nav .sitebtn{background:transparent;color:var(--linen);border:1px solid var(--line-2);border-radius:999px;padding:6px 12px;font:600 12px var(--ui);cursor:pointer;white-space:nowrap}.nav .sitebtn:hover{border-color:var(--straw);color:var(--straw)}.nav .sitemenu{position:absolute;top:calc(100% + 6px);left:0;z-index:2000;min-width:230px;background:var(--night);border:1px solid var(--line-2);border-radius:10px;box-shadow:var(--sh);padding:6px}.nav .sitemenu a{display:flex;flex-direction:column;gap:2px;padding:8px 10px;border-radius:7px;color:var(--linen);text-decoration:none;font:600 12.5px var(--ui)}.nav .sitemenu a small{color:var(--muted);font-weight:400;font-size:11px}.nav .sitemenu a:hover,.nav .sitemenu a.on{background:var(--night-2)}@media(max-width:700px){.nav .sitebtn{padding:6px 9px;font-size:11px}}.nav .edit{display:inline-flex;align-items:center;gap:7px;margin-right:8px;background:var(--night-2);color:var(--linen);border:1px solid var(--line-2);border-radius:999px;padding:8px 14px;font:700 12px var(--display);letter-spacing:.06em;cursor:pointer}.nav .edit svg{width:14px;height:14px}.nav .edit:hover{border-color:var(--straw);color:var(--straw)}.nav .edit.on{background:var(--straw);color:var(--bark);border-color:var(--straw)}@media(max-width:700px){.nav .edit span{display:none}.nav .edit{padding:8px 10px}}`; document.head.appendChild(est);
  window.openShare = openSheet;
  // another property: page headings that name EdenRise as the estate take the property name (the nav brand stays EdenRise = the console)
  if(window.SITE && SITE_ID !== "edenrise"){
    const nm = SITE.name; const re = /Herdade EdenRise|Herdade do EdenRise|EdenRise — |EdenRise/g;
    const fix = () => { if(/EdenRise/.test(document.title)) document.title = document.title.replace(/EdenRise — |EdenRise/g, m => m === "EdenRise — " ? nm + " — " : nm);
      document.querySelectorAll("h1,h2,.eyebrow,.sub,.hero p,.lead").forEach(el => { if(el.closest("#appnav") || !/EdenRise/.test(el.textContent)) return; el.innerHTML = el.innerHTML.replace(re, m => m === "EdenRise — " ? nm + " — " : nm); }); };
    fix(); setTimeout(fix, 800); setTimeout(fix, 2500);
  }
})();
