...
function snapLL(ll){ if(!gridOn||!SH.snap) return ll; const P=proj(GRID_ORIGIN), sp=gridSp; const [x,y]=P.to(ll); const gx=Math.round(x/sp)*sp, gy=Math.round(y/sp)*sp; const m=mpp(), tn=SNAP_PX*m, tl=LINE_PX*m; const dx=Math.abs(x-gx), dy=Math.abs(y-gy);
