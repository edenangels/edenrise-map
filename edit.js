...
const TOOLS=[["point",T.point],["line",T.line],["area",T.area],["rect",T.rect],["circle",T.circle],["shape",T.shape],null,["select",T.select],["move",T.move],["rotate",T.rotate],["undo",T.undo],null,["x",T.exit]];
...
tools.addEventListener("click",e=>{ const b=e.target.closest("button"); if(!b) return; const t=b.dataset.t; if(t==="x") return exit(); if(t==="undo") return undo(); if(t==="shape") return openShapes(); setTool(E.tool===t&&SHAPE[t]?null:t); });
...
map.on("pm:create",e=>{ map.pm.disableDraw(); let l=e.layer; if(e.shape==="Rectangle"){ const ll=l.getLatLngs(); map.removeLayer(l); l=L.polygon(ll,{color:"#c9a227",weight:3,fillColor:"#c9a227",fillOpacity:.15}); }
...
function startEdit(l,ctx){ if(E.cur&&E.cur!==l&&work.hasLayer(E.cur)&&!E.cur.__keep) work.removeLayer(E.cur); E.cur=l; E.undo=[]; E.ctx=ctx; E.tool=null; tools.querySelectorAll("button").forEach(b=>b.classList.remove("on")); mapEl.classList.remove("ed-draw");
  ["pm:edit","pm:dragend","pm:rotateend","pm:markerdragend","pm:vertexremoved"].forEach(ev=>l.on(ev,refreshMeasures)); ["pm:dragstart","pm:rotatestart","pm:vertexadded","pm:markerdragstart"].forEach(ev=>l.on(ev,snapshot));
...
async function refreshMeasures(){ if(!E.cur||form.hidden) return; const m=measure(E.cur); M=m; const el=form.querySelector("#emeas"); if(!el) return; const row=(k,v,u="")=>v==null?"":`<div><span>${k}</span><b>${typeof v==="number"?fmt(v,v>=100?0:v>=10?1:2):v}${u}</b></div>`;
...
function closeForm(){ form.hidden=true; if(E.cur){ try{ E.cur.pm&&E.cur.pm.disable(); }catch(e){} if(work.hasLayer(E.cur)) work.removeLayer(E.cur); if(E.cur.__origLayer){ try{ E.cur.__origLayer.setStyle(E.cur.__origLayer.__orig||{}); }catch(e){} } } E.cur=null; E.ctx=null; }
...
function snapLL(ll){ if(!gridOn||!SH.snap) return ll; const P=proj(GRID_ORIGIN), sp=grid.__sp||gridSp; const [x,y]=P.to(ll); return P.from(Math.round(x/sp)*sp, Math.round(y/sp)*sp); }
...
function endPlace(){ if(placeMode==="tap") map.off("click",onTapPlace); map.off("mousemove",onMovePreview); if(placeMode==="drag"){ map.dragging.enable(); const el=map.getContainer(); el.removeEventListener("touchstart",dragStart); el.removeEventListener("touchmove",dragMove); el.removeEventListener("touchend",dragEnd); map.off("mousedown",dragStart); map.off("mousemove",dragMove); map.off("mouseup",dragEnd); }
  if(preview){ map.removeLayer(preview); preview=null; } placeMode=null; mapEl.classList.remove("ed-draw"); }
function drop(c){ endPlace(); const l=buildShape(c); work.addLayer(l); startEdit(l,{preset:null,shape:SH.kind}); showHint(null); }
function onMovePreview(e){ const c=snapLL(e.latlng); if(preview) map.removeLayer(preview); preview=buildShape(c,{dashArray:"6 5",fillOpacity:.08,interactive:false}); preview.addTo(map); }
function onTapPlace(e){ drop(snapLL(e.latlng)); }
let dragA=null;
...
function dragMove(e){ if(!dragA) return; const ll=evLL(e); if(!ll) return; const B=snapLL(ll); const P=proj(dragA); const [dx,dy]=P.to(B); SH.a=Math.abs(dx); SH.b=Math.abs(dy); SH.rot=0; if(preview) map.removeLayer(preview); preview=L.rectangle(L.latLngBounds(dragA,B),{color:"#c9a227",weight:2,dashArray:"6 5",fillColor:"#c9a227",fillOpacity:.08,interactive:false}).addTo(map); showHint(`${fmt(SH.a,2)} × ${fmt(SH.b,2)} m · ${fmt(SH.a*SH.b,1)} m²`); if(e.preventDefault) e.preventDefault(); }
...
function openShapes(){ setTool(null); closeForm(); form.hidden=false; const u=SH.unit, k=u==="cm"?100:1, U=u; const num=(id,v,lab)=>`<label>${lab} (${U})<input id="${id}" type="number" step="${u==="cm"?1:0.01}" value="${+(v*k).toFixed(2)}"></label>`;
  ...
  form.querySelector("#sgrid").onchange=e=>{ rd(); const v=+e.target.value; gridOn=v>0; if(gridOn){ gridSp=v; if(!map.hasLayer(grid)) grid.addTo(map); } else if(map.hasLayer(grid)) map.removeLayer(grid); drawGrid(); openShapes(); };
  ...
  form.querySelector("#splace").onclick=()=>{ rd(); endPlace(); placeMode="tap"; mapEl.classList.add("ed-draw"); showHint(T.place); map.on("mousemove",onMovePreview); map.once("click",onTapPlace); };
