...
const btn = document.createElement("button"); btn.id="brainbtn"; btn.setAttribute("aria-label",T.title); btn.innerHTML=`<span class="orb"></span>${T.title}`; document.getElementById("map").appendChild(btn);
const box = document.createElement("div"); box.id="brain"; box.hidden=true; box.setAttribute("role","dialog"); box.setAttribute("aria-label",T.title); box.innerHTML=`<div class="hd"><span class="orb" style="width:12px;height:12px;border-radius:50%;background:var(--oak)"></span><div style="flex:1"><h3>${T.title}</h3><div class="sub">${T.sub}</div></div><span class="role ${ROLE}" id="brole">${T.viewer}</span><button id="bclose" style="background:transparent;border:0;color:var(--muted);font-size:16px;cursor:pointer">✕</button></div>
